...
export function parseClass(name) {
    ...
    if (name.startsWith("pr-"))
        return { paddingRight: toSpacing(name.slice(3)) };
    if (name.startsWith("pb-"))
        return { paddingBottom: toSpacing(name.slice(3)) };
    if (name.startsWith("pl-"))
        return { paddingLeft: toSpacing(name.slice(3)) };
    ...
    if (name.startsWith("mr-"))
        return { marginRight: toSpacing(name.slice(3)) };
    if (name.startsWith("mb-"))
        return { marginBottom: toSpacing(name.slice(3)) };
    ...
    if (name.startsWith("rounded-"))
        return { borderRadius: BORDER_RADIUS[name.slice(8)] || name.slice(8) };
    ...
    if (name.startsWith("cols-"))
        return {
            gridTemplateColumns: `repeat(${name.slice(5)}, minmax(0, 1fr))`,
        };
