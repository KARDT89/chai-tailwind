...
export function parseClass(name) {
    ...
    if (name.startsWith("border-")) {
        ...
        if (BORDER_WIDTHS[v]) return { borderWidth: BORDER_WIDTHS[v], borderStyle: "solid" };
    ...
    if (name.startsWith("self-")) {
        const map = { auto: "auto", start: "flex-start", end: "flex-end", center: "center", stretch: "stretch" };
    ...
    // ── Transitions ───────────────────────────────────────────────────────────
    if (name === "transition")         return { transitionProperty: "color, background-color, border-color, text-decoration-color, fill, stroke, opacity, box-shadow, transform, filter, backdrop-filter", transitionTimingFunction: TRANSITIONS.easing["in-out"], transitionDuration: TRANSITIONS.duration[150] };
    if (name === "transition-none")    return { transitionProperty: "none" };
    if (name === "transition-all")     return { transitionProperty: "all", transitionTimingFunction: TRANSITIONS.easing["in-out"], transitionDuration: TRANSITIONS.duration[150] };
    if (name === "transition-colors")  return { transitionProperty: "color, background-color, border-color", transitionTimingFunction: TRANSITIONS.easing["in-out"], transitionDuration: TRANSITIONS.duration[150] };
    if (name === "transition-opacity") return { transitionProperty: "opacity", transitionTimingFunction: TRANSITIONS.easing["in-out"], transitionDuration: TRANSITIONS.duration[150] };
    if (name === "transition-shadow")  return { transitionProperty: "box-shadow", transitionTimingFunction: TRANSITIONS.easing["in-out"], transitionDuration: TRANSITIONS.duration[150] };
    if (name === "transition-transform") return { transitionProperty: "transform", transitionTimingFunction: TRANSITIONS.easing["in-out"], transitionDuration: TRANSITIONS.duration[150] };

    if (name.startsWith("duration-"))
        return { transitionDuration: TRANSITIONS.duration[name.slice(9)] || `${name.slice(9)}ms` };
    if (name.startsWith("ease-"))
        return { transitionTimingFunction: TRANSITIONS.easing[name.slice(5)] || name.slice(5) };
    if (name.startsWith("delay-"))
        return { transitionDelay: TRANSITIONS.duration[name.slice(6)] || `${name.slice(6)}ms` };
    ...
    if (name === "antialiased")   return { WebkitFontSmoothing: "antialiased", MozOsxFontSmoothing: "grayscale" };
